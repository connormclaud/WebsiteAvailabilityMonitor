...
async def test_check_website_success():
    async with aiohttp.ClientSession() as session:
        result = await check_website(session, "http://localhost:8080/get")

        assert result["url"] == "http://localhost:8080/get"
...
async def test_check_website_failure():
    async with aiohttp.ClientSession() as session:
        result = await check_website(session, "http://localhost:8080/status/404")

        assert result["url"] == "http://localhost:8080/status/404"
        assert 0 <= result["response_time"] <= 5
...
async def test_check_website_content_match():
    async with aiohttp.ClientSession() as session:
        ...
        result = await check_website(session, "http://localhost:8080/html", regexp=pattern)

        assert result["url"] == "http://localhost:8080/html"
...
async def test_main_function():
    ...
    with patch('website_checker.check_website', side_effect=mock_check_website) as mock_check_website:
        with patch('website_checker.asyncio.sleep', new=mock_sleep):
            from src.website_checker import main
